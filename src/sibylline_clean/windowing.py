...
class SlidingWindowAnalyzer:
    ...
    def _compute_window_score(self, signal: MotifSignal) -> float:
        ...
        if signal.category_scores:
            avg_category = sum(signal.category_scores.values()) / (
                100 * len(signal.category_scores)
            )
        else:
            avg_category = 0.0

        # Weighted combination
        return 0.4 * density_score + 0.3 * coverage_score + 0.3 * avg_category
    ...
    def get_regions_for_embedding(
        ...
        for start, end in result.batched_regions:
            ...
            if min_score > 0:
                region_score = max(
                    (h.peak_score for h in result.hotspots if h.start >= start and h.end <= end),
                    default=0,
                )
